...
async function getRecentlyAdded(): Promise<ComponentSummary[] | undefined> {
    try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/apps/recently_added`, { next: { revalidate: 600 }});
...
async function getRecentlyUpdated(): Promise<ComponentSummary[] | undefined> {
    try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/apps/recently_updated`, { next: { revalidate: 600 }});
