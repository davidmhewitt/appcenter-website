...
async function getData(): Promise<App[] | undefined> {
    try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/dashboard/apps`, { cache: 'no-store' });
                                    ...
                                    {apps?.map(({ id, is_verified, version }, index) => (
                                        <tr key={index}
                                            ...
                                            <td className="whitespace-nowrap px-4 py-3 font-medium">
                                                {id}{is_verified && (<CheckBadgeIcon className='w-6 h-6 inline ms-1' />)}
                                            </td>
                                            <td className="whitespace-nowrap px-4 py-3">{version}</td>
                                            <td className="whitespace-nowrap px-4 py-3">{is_verified && (<CloudArrowUpIcon className='w-6 h-6' />)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
